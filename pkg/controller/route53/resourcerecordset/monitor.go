...
func eventsFromMonitor(mgr ctrl.Manager, mnt monitor.Monitor) source.Source {
	mnt.AddPrepareHook(func(ctx context.Context) error {
		if err := cache.IndexByExternalName(ctx, mgr.GetCache(), &route53v1alpha1.ResourceRecordSet{}); err != nil {
			return fmt.Errorf("setting up indexer: %w", err)
		}
		return nil
	})

	eventsChannel := make(chan event.GenericEvent, 10)
	mnt.AddSubscriber(func(ctx context.Context, evt monitor.Event) {
		if evt.GVK != route53v1alpha1.ResourceRecordSetGroupVersionKind {
			return
		}
		var list route53v1alpha1.ResourceRecordSetList
