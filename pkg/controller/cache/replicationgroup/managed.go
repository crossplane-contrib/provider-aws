...
func (e *external) Observe(ctx context.Context, mg resource.Managed) (managed.ExternalObservation, error) { //nolint:gocyclo
	...
	e.cache.tagsNeedUpdate = tagsNeedUpdate
...
func (e *external) Update(ctx context.Context, mg resource.Managed) (managed.ExternalUpdate, error) { //nolint:gocyclo
	...
	if cr.GetWriteConnectionSecretToReference() != nil {
		authTokenChanged = e.cache.currentAuthToken != e.cache.desiredAuthToken ||
			cr.Status.AtProvider.LastAppliedAuthTokenUpdateStrategy != cr.Spec.ForProvider.AuthTokenUpdateStrategy
	}

	if diff := elasticache.ReplicationGroupNeedsUpdate(cr.Spec.ForProvider, rg, ccList); diff != "" || authTokenChanged {
	...
	if e.cache.tagsNeedUpdate {
		err = e.updateTags(ctx, cr.Spec.ForProvider.Tags, rg.ARN)
		if err != nil {
			var invalidState *awselasticachetypes.InvalidReplicationGroupStateFault
			if errors.As(err, &invalidState) {
				// Cluster is not in a state to accept tag operations (e.g. resharding in progress).
				// Return without error — the next reconcile will retry once the cluster is ready.
