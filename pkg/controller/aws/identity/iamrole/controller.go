...
func (c *Controller) SetupWithManager(mgr ctrl.Manager) error {
	r := resource.NewManagedReconciler(mgr,
		resource.ManagedKind(v1alpha2.IAMRoleGroupVersionKind),
	...
	name := strings.ToLower(fmt.Sprintf("%s.%s", v1alpha2.IAMRoleKindAPIVersion, v1alpha2.Group))
	return ctrl.NewControllerManagedBy(mgr).
		Named(name).
		For(&v1alpha2.IAMRole{}).
		Complete(r)
}
