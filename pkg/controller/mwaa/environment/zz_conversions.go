...
func GenerateEnvironment(resp *svcsdk.GetEnvironmentOutput) *svcapitypes.Environment {
	...
	if resp.Environment.MaxWebservers != nil {
		cr.Spec.ForProvider.MaxWebservers = resp.Environment.MaxWebservers
	} else {
		cr.Spec.ForProvider.MaxWebservers = nil
	}
	...
	if resp.Environment.MinWebservers != nil {
		cr.Spec.ForProvider.MinWebservers = resp.Environment.MinWebservers
	} else {
		cr.Spec.ForProvider.MinWebservers = nil
	}
	...
	if resp.Environment.Tags != nil {
		f28 := map[string]*string{}
		for f28key, f28valiter := range resp.Environment.Tags {
			var f28val string
			f28val = *f28valiter
			f28[f28key] = &f28val
		}
...
func GenerateCreateEnvironmentInput(cr *svcapitypes.Environment) *svcsdk.CreateEnvironmentInput {
	...
	if cr.Spec.ForProvider.MaxWebservers != nil {
		res.SetMaxWebservers(*cr.Spec.ForProvider.MaxWebservers)
	}
	if cr.Spec.ForProvider.MaxWorkers != nil {
		res.SetMaxWorkers(*cr.Spec.ForProvider.MaxWorkers)
	}
	if cr.Spec.ForProvider.MinWebservers != nil {
		res.SetMinWebservers(*cr.Spec.ForProvider.MinWebservers)
	}
