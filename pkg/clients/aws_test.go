...
func TestUseProviderConfig(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"DynamicURLConfig": {
			...
			want: want{
				url: "https://iam.us-east-1.amazonaws.com",
			},
		},
		"Localstack": {
			args: args{
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if tc.args.endpointConfig != nil {
				actual, endpointError := config.EndpointResolver.ResolveEndpoint("iam", tc.args.region)
