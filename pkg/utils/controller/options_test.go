...
func TestOptionsOverrides(t *testing.T) {
	...
	options.AddOverrides(map[string]string{
		...
		"ec2.instance.pollIntervalJitter": "10s",
	...
	if diff := cmp.Diff(0*time.Second, options.Default().PollIntervalJitter); diff != "" {
		t.Errorf("default.PollIntervalJitter: -want, +got:\n%s", diff)
	}
	if diff := cmp.Diff(3, options.Default().MaxConcurrentReconciles); diff != "" {
		t.Errorf("default.MaxConcurrentReconciles: -want, +got:\n%s", diff)
	}

	// overrides with dot in the scope name
	if diff := cmp.Diff(30*time.Second, options.Get("ec2.instance").PollInterval); diff != "" {
		t.Errorf("ec2.instance.PollInterval: -want, +got:\n%s", diff)
	}
	if diff := cmp.Diff(10*time.Second, options.Get("ec2.instance").PollIntervalJitter); diff != "" {
		t.Errorf("ec2.instance.PollIntervalJitter: -want, +got:\n%s", diff)
	}
	...
	if diff := cmp.Diff(0*time.Second, options.Get("route53").PollIntervalJitter); diff != "" {
		t.Errorf("route53.PollIntervalJitter: -want, +got:\n%s", diff)
	}
	...
	if diff := cmp.Diff(0*time.Second, options.Get("sqs").PollIntervalJitter); diff != "" {
		t.Errorf("sqs.PollIntervalJitter: -want, +got:\n%s", diff)
	}
